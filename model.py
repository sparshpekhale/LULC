...
from utils import FocalLoss, calculate_metrics, calculate_class_metrics
...
class MACUNet_model(nn.Module):
    def __init__(self, band_num, class_num):
        super(MACUNet_model, self).__init__()
...
class MACUNet(pl.LightningModule):
    def __init__(self, band_num, num_classes):
        ...
        self.model = MACUNet_model(band_num, num_classes)
    ...
    def training_step(self, batch, batch_idx):
        ...
        soy_acc, _, _, soy_iou = calculate_class_metrics(y, pred, 3) # Soybeans = 3
        self.soy_accs.append(soy_acc)
        self.soy_ious.append(soy_iou)
    ...
    def on_train_epoch_end(self):
        ...
        avg_iou = torch.stack(self.ious).mean()
        avg_soy_acc = torch.tensor(self.soy_accs).float().mean()
        avg_soy_iou = torch.tensor(self.soy_ious).float().mean()

        self.log('train_loss', avg_loss)
        self.log('train_acc', avg_acc)
        self.log('train_iou', avg_iou)
        self.log('train_soy_acc', avg_soy_acc)
        self.log('train_soy_iou', avg_soy_iou)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        soy_acc, _, _, soy_iou = calculate_class_metrics(y, pred, 3) # Soybeans = 3
        self.val_soy_accs.append(soy_acc)
        self.val_soy_ious.append(soy_iou)
    ...
    def on_validation_epoch_end(self):
        ...
        avg_soy_acc = torch.tensor(self.val_soy_accs).float().mean()
        avg_soy_iou = torch.tensor(self.val_soy_ious).float().mean()

        self.log('val_loss', avg_loss)
        self.log('val_acc', avg_acc)
        self.log('val_iou', avg_iou)
        self.log('val_soy_acc', avg_soy_acc)
        self.log('val_soy_iou', avg_soy_iou)

        self.val_losses.clear()
        self.val_accs.clear()
        self.val_ious.clear()
        self.val_soy_accs.clear()
        self.val_soy_ious.clear()

    def configure_optimizers(self):
        optim = torch.optim.Adam(self.model.parameters(), lr=0.0003)
        scheduler = ReduceLROnPlateau(optim, mode='min', factor=0.5, patience=20, min_lr=1e-6, verbose=True)
        return {
            'optimizer': optim,
            'lr_scheduler': {
                'scheduler': scheduler,
...
if __name__ == '__main__':
    num_classes = 10
    in_batch, inchannel, in_h, in_w = 2, 6, 224, 224
